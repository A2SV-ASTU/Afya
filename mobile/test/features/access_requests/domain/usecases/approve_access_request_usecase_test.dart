...
void main() {
  late ApproveAccessRequestUseCase useCase;
  late MockAccessRequestRepository mockRepository;

  setUp(() {
    mockRepository = MockAccessRequestRepository();
    useCase = ApproveAccessRequestUseCase(mockRepository);
  });

  const tRequestId = '1';

  group('ApproveAccessRequestUseCase', () {
    test('should approve access request from repository', () async {
      when(() => mockRepository.approveAccessRequest(tRequestId))
          .thenAnswer((_) async => const Right(unit));

      final result = await useCase(tRequestId);

      expect(result, const Right(unit));
      verify(() => mockRepository.approveAccessRequest(tRequestId)).called(1);
