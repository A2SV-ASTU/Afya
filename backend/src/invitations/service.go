...
type Service interface {
	CreateInvitation(ctx context.Context, clinicID, invitedBy uuid.UUID, req CreateInvitationRequest, apiBaseURL string) error
...
func NewService(db *sql.DB, repo Repository, sender *email.Sender, cfg *config.Config) Service {
	return &service{
		db:     db,
		repo:   repo,
		sender: sender,
...
func (s *service) CreateInvitation(ctx context.Context, clinicID, invitedBy uuid.UUID, req CreateInvitationRequest, apiBaseURL string) error {
	...
	if s.sender != nil {
		// Build magic link URL for the doctor to accept the invitation
		acceptURL := fmt.Sprintf("%s/api/v1/magic/accept-invitation?token=%s", apiBaseURL, rawToken)
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Click the button below to accept your invitation and set up your profile:</p>
            <div class="btn-group">
                <a href="%s" class="btn">Accept Invitation</a>
            </div>
            <div class="alert">
                <strong>Important:</strong> This invitation link will expire in exactly 24 hours.
            </div>
        </div>
        <div class="footer">
