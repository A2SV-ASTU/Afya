...
const reqSelectWithUser = `ar.id, ar.patient_id, ar.requesting_clinic_id, ar.reason, ar.submitted_by_doctor_id, ar.status, ar.expires_at, ar.revoked_at, ar.created_at, ar.updated_at, ar.token_hash, u.first_name, u.last_name, u.email`
...
func (r *repository) FindByTokenHash(ctx context.Context, tokenHash string) (*AccessRequest, error) {
	query := fmt.Sprintf(`SELECT %s FROM access_requests ar LEFT JOIN users u ON u.id = ar.patient_id WHERE ar.token_hash = $1`, reqSelectWithUser)
...
func (r *repository) FindByID(ctx context.Context, id uuid.UUID) (*AccessRequest, error) {
	query := fmt.Sprintf(`SELECT %s FROM access_requests ar LEFT JOIN users u ON u.id = ar.patient_id WHERE ar.id = $1`, reqSelectWithUser)
...
func (r *repository) ListByClinicID(ctx context.Context, clinicID uuid.UUID, status string) ([]*AccessRequest, error) {
	...
	var args []any
	if status != "" {
		query = fmt.Sprintf(`SELECT %s FROM access_requests ar LEFT JOIN users u ON u.id = ar.patient_id WHERE ar.requesting_clinic_id = $1 AND ar.status = $2 ORDER BY ar.created_at DESC`, reqSelectWithUser)
		args = []any{clinicID, status}
	} else {
		query = fmt.Sprintf(`SELECT %s FROM access_requests ar LEFT JOIN users u ON u.id = ar.patient_id WHERE ar.requesting_clinic_id = $1 ORDER BY ar.created_at DESC`, reqSelectWithUser)
		args = []any{clinicID}
	}
...
func (r *repository) FindActiveGrant(ctx context.Context, clinicID, patientID uuid.UUID) (*AccessRequest, error) {
	query := fmt.Sprintf(`
		SELECT %s FROM access_requests ar
		LEFT JOIN users u ON u.id = ar.patient_id
		WHERE ar.requesting_clinic_id = $1 AND ar.patient_id = $2
		AND ar.status = 'approved' AND ar.revoked_at IS NULL
