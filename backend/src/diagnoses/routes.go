...
func RegisterRoutes(router *gin.RouterGroup, handler *Handler, db *sql.DB, jwtSecret string) {
	group := router.Group("/encounters/:encounterId/diagnoses")
	group.Use(middleware.RequireAuth(jwtSecret))
	group.Use(EncounterAccessGuard(db))
	{
		group.POST("", middleware.RequireRole(string(users.RoleDoctor)), handler.CreateDiagnosis)
		group.GET("", middleware.RequireRole(string(users.RoleDoctor), string(users.RolePatient)), handler.GetDiagnoses)
	}
}
